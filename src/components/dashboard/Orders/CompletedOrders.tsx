...
const CompletedOrders = ({ isShow, setIsShow, handleOpenModal }: any) => {
  const [order, setOrder] = useState<any>({});
  const [items, setItems] = useState<any>([]);
  const [currentIndex, setCurrentIndex] = useState<any>(0);

  const orders: any = useGetAllOrders({
    status: "completed",
    isShowModal: isShow,
  });
    ...
    >
      <DrawerContent>
        <DrawerHeader className="flex gap-1">
          Completed Orders <Chip color="success">{orders?.length || 0}</Chip>
          <ModalMenuBar
                    ...
                    <Tab
                      ...
                      title={
                        <div className=" space-x-2">
                          <span>{item?.username || `User (${index + 1})`}</span>
