...
import EditCandidate from "./forms/EditCandidate";
...
const CandidateShortlist: React.FC = () => {
  ...
  const candidates: any = useGetAllCandidates({
    ...
    filters: { isShortlist: true, search: debouncedValue },
  });
        ...
        const receiveData: editMenuResponse = await window.ipcRenderer.invoke(
          "update-candidate",
          {
            updateData: JSON.parse(
              JSON.stringify({ isShortlist: false, isSelected: false })
            ),
            _id: id,
          }
        );
  ...
  return (
    <div className="p-4 mt-16">
      <h1 className="text-2xl font-semibold mb-4">Shortlist Candidates</h1>
