...
export default function ManageAward(): JSX.Element {
    ...
    <div className="min-h-screen bg-gray-100 p-10">
      <div className="max-w-6xl mx-auto bg-white p-8 shadow-md rounded-md">
        <div className="flex justify-between">
          <h1 className="text-2xl font-bold mb-5">Manage Award</h1>
        ...
        <Table className="min-w-full table-auto border-collapse">
          <TableHeader className="bg-gray-100">
            ...
            <TableColumn className="border p-2 text-left cursor-pointer">
              <span onClick={() => handleSort("expense_type")}>Award Name</span>
            ...
            <TableColumn className="border p-2 text-left cursor-pointer">
              <span onClick={() => handleSort("amount")}>Gift Item</span>
              {sortConfig.key === "amount" &&
                (sortConfig.direction === "asc" ? "🔼" : "🔽")}
            </TableColumn>
            <TableColumn className="border p-2 text-left cursor-pointer">
              <span onClick={() => handleSort("amount")}>Date</span>
              {sortConfig.key === "amount" &&
                (sortConfig.direction === "asc" ? "🔼" : "🔽")}
            </TableColumn>
            <TableColumn className="border p-2 text-left cursor-pointer">
              <span onClick={() => handleSort("amount")}>Employee Name</span>
              {sortConfig.key === "amount" &&
                (sortConfig.direction === "asc" ? "🔼" : "🔽")}
            </TableColumn>
            <TableColumn className="border p-2 text-left cursor-pointer">
              <span onClick={() => handleSort("amount")}>Award By</span>
              {sortConfig.key === "amount" &&
                (sortConfig.direction === "asc" ? "🔼" : "🔽")}
            </TableColumn>
          ...
          <TableBody>
            {paginatedData().map((item: any, index: number) => (
              <TableRow key={item._id} className="border-b">
                ...
                <TableCell className="border p-2">{item.amount}</TableCell>
                <TableCell className="border p-2">{item.amount}</TableCell>
                <TableCell className="border p-2">{item.amount}</TableCell>
                <TableCell className="border p-2">{item.amount}</TableCell>
                <TableCell className="border p-2">{item.amount}</TableCell>
