...
export default function EmployeeSalary(): JSX.Element {
    ...
    <div className="min-h-screen bg-gray-100 p-10">
      <div className="max-w-6xl mx-auto bg-white p-8 shadow-md rounded-md">
        <div className="flex justify-between">
          <h1 className="text-2xl font-bold mb-5">Employee Salary List</h1>
          <div className="flex gap-3 items-center">
            ...
            >
              <PopoverTrigger>
                <Button color="secondary">Filter Staff</Button>
              </PopoverTrigger>
              <PopoverContent>
                <Listbox onAction={(key) => setSelectedRoles(key)}>
                  <ListboxItem key="" className="!mb-0">
                    All
                  </ListboxItem>
                </Listbox>
                <Listbox
                  aria-label="Actions"
                  onAction={(key) => setSelectedRoles(key)}
                >
                  {stuffRoles.map((item: any) => (
                    <ListboxItem key={item.role} className="!mb-0">
                      {item.role}
                    </ListboxItem>
                  ))}
                </Listbox>
