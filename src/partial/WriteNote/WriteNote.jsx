...
function WriteNote({ setIsShow, isShow, targetId, noteData }) {
  const [text, setText] = useState < any > (noteData || "");
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const response = await axiosSecure.put(
        `/users/put/${targetId}?branch=${branch}`,
        { notes: text }
      );
  ...
  return (
    <article
      className={`w-full md:w-[65%] lg:w-[50%] rounded-xl bg-white my-7 transition-all duration-500 ${
    ...
    >
      ...
      <form className="px-5 py-3" onSubmit={onSubmit}>
        ...
        <div className="flex justify-end items-center gap-3">
          <div className="flex gap-3 cursor-pointer items-center hover:bg-gray-700 hover:text-white py-1 px-3 rounded-xl  border hover:border-transparent  text-gray-700  border-gray-300 transition duration-300">
            <button
